"""
delete_task tool handler for MCP tools.
Implements the delete_task functionality that allows AI agents to delete tasks.
"""
...
from ..models.tool_requests import DeleteTaskRequest, DeleteTaskResponse
...
class DeleteTaskHandler:
    ...
    Processes requests to delete tasks via the MCP protocol.
    """
    ...
    async def handle(self, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Handle a delete_task tool call from an AI agent.
        ...
        try:
            ...
            response = await self.tool_executor.execute_delete_task(request)

            # Log the successful operation
            logger.info(f"Successfully deleted task {response.task_id} for user {request.user_id}")
...
async def handle_delete_task(tool_executor: ToolExecutor, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Convenience function to handle delete_task tool calls.
    ...
    Returns:
        List containing the response from the tool execution
    """
    handler = DeleteTaskHandler(tool_executor)
